if __name__ == '__main__':
    ...
    file_handle=open('TimeLogCarbon.txt')
    ...
    for line in file_handle:
        ...
        if line.find(':')==-1:
            ...
            #hours spent at that interval
            time_spent=GetTimeValue(GetTimePeriod(line))
            x=hours_time(time_spent)
            y=minutes_time(time_spent)
            hours.append(x)
            mins.append(y)        

    min_to_hours=int(sum(mins)/len(mins))
    final_hours=sum(hours)+min_to_hours
    dec=str(sum(mins)/len(mins))
    dot=dec.find('.')
    space=dec.find(' ')
    deci=dec[dot-1:space]
    print(deci)
    remainder_mins=float(deci)*60
    print('Hours Spent on file is: ',final_hours)
    print('Minutes Spent on file is: ',int(remainder_mins))